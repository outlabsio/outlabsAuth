...
@router.post("/", status_code=status.HTTP_201_CREATED, dependencies=[Depends(has_permission("client_account:create"))])
async def create_client_account(
    ...
    return ClientAccountResponseSchema(**account_dict)
...
async def get_all_client_accounts(
    ...
    skip: int = 0,
    limit: int = 100
) -> List[ClientAccountResponseSchema]:
    ...
    for account in accounts:
        ...
        if account_dict.get("main_contact_user_id"):
            account_dict["main_contact_user_id"] = str(account_dict["main_contact_user_id"])
        converted_accounts.append(ClientAccountResponseSchema(**account_dict))
...
@router.put("/{account_id}", dependencies=[Depends(has_permission("client_account:update"))])
