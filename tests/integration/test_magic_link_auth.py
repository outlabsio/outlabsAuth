...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_magic_link_request_rate_limit_returns_retry_metadata(test_engine):
    auth = SimpleRBAC(
        engine=test_engine,
        secret_key="test-secret-key-do-not-use-in-production-12345678",
        enable_magic_links=True,
        magic_link_request_rate_limit_max=1,
    ...
    try:
        app = _build_app(auth)
        transport = httpx.ASGITransport(app=app)
        email = f"limited-magic-{uuid.uuid4().hex[:8]}@example.com"
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
