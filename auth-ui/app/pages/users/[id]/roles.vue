...
const { data: allRoles, isLoading: isLoadingRoles } = useQuery(rolesQueries.list())
...
const availableRoles = computed(() => {
  ...
  return allRoles.value.items?.filter(r => !assignedRoleIds.includes(r.id)) || []
})
...
async function handleAddRole() {
  ...
  const success = await userStore.assignRole(props.user.id, selectedRoleId.value)
  ...
  <UCard>
    <template #header>
      <div class="flex items-center justify-between">
        <div>
          <h3 class="text-lg font-semibold text-foreground">Role Assignments</h3>
          <p class="text-sm text-muted">Manage global roles for this user</p>
        </div>
    ...
    <div class="space-y-3 mb-6">
      ...
      <div v-else-if="userRoles.length === 0" class="text-center py-8">
        <UIcon name="i-lucide-shield-off" class="w-12 h-12 text-muted mb-4" />
        <p class="text-sm font-medium text-foreground mb-1">No roles assigned</p>
        <p class="text-xs text-muted">This user has no roles assigned yet</p>
      </div>

      <UCard v-else v-for="membership in userRoles" :key="membership.role.id" class="hover:bg-muted/50 transition-colors">
        <div class="flex items-start justify-between">
          ...
          <div class="flex-1">
            <div class="flex items-center gap-2 mb-1">
              ...
              <UBadge v-if="membership.role.is_global" color="blue" variant="subtle">
                Global
              </UBadge>
            </div>

            <p v-if="membership.role.description" class="text-sm text-muted mb-2">
            ...
            <div class="flex items-center gap-4 text-xs text-muted">
              <div class="flex items-center gap-1">
                ...
                <span>{{ membership.role.permissions?.length || 0 }} permissions</span>
              </div>
              <div class="flex items-center gap-1">
                <UIcon name="i-lucide-calendar" class="w-3 h-3" />
                <span>Granted {{ formatDate(membership.granted_at) }}</span>
              </div>
            </div>
          </div>
    ...
    <div class="border-t border-border pt-6">
      ...
      <div v-else-if="availableRoles.length === 0" class="text-center py-4">
        <p class="text-sm text-muted">All available roles have been assigned</p>
      </div>
      ...
      <p class="text-xs text-muted mt-2">
        Assigning a role grants all permissions associated with that role
      </p>
    </div>
  </UCard>
</template>
