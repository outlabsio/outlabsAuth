...
export function useCreateEntityMutation() {
  ...
  return useMutation({
    ...
    onSuccess: (newEntity) => {
      ...
      if (newEntity.parent_entity_id) {
        queryClient.invalidateQueries({
          key: ENTITY_KEYS.hierarchy(newEntity.parent_entity_id),
        });
      }
...
export function useMoveEntityMutation() {
  ...
  return useMutation({
    ...
    onSuccess: ({ entityId, newParentId, oldParentId }) => {
      ...
      if (newParentId) {
        queryClient.invalidateQueries({
          key: ENTITY_KEYS.hierarchy(newParentId),
        });
      }
      ...
      if (oldParentId && oldParentId !== newParentId) {
        queryClient.invalidateQueries({
          key: ENTITY_KEYS.hierarchy(oldParentId),
        });
      }
