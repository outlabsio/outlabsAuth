...
- ✅ **User handbook polish (docs-library)** — handbook home + reading paths; Introduction / Choosing a Preset; rewritten User Management API against live routers; humanized invitations, status, API keys, JWT openings; audience headers across topic guides; trimmed Activity Tracking / Testing / Memberships; Entities (`51`) + Roles & Permissions (`25`) + ABAC (`26`); tightened Core Authorization Concepts; Nuxt docs IA sketch in handbook README. Content source for a future Nuxt docs site.
