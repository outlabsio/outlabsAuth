...
class NotificationService:
    ...
    async def emit(
        ...
        asyncio.create_task(
            self._process_event(event_type, data, metadata)
        )
    ...
    async def _process_event(
        ...
        try:
            ...
            if tasks:
                ...
                # return_exceptions=True means exceptions don't propagate
                await asyncio.gather(*tasks, return_exceptions=True)
                
        except Exception:
            # Never let notification errors crash the app
            # In production, you might want to log this to an error
            # tracking service (Sentry, etc.)
            pass
