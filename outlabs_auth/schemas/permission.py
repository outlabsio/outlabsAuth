...
class PermissionCreateRequest(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=100, description="Permission name in format resource:action")
    display_name: str = Field(..., min_length=1, max_length=200, description="Human-readable permission name")
    description: Optional[str] = Field(None, max_length=1000, description="Detailed description of what this permission allows")
    is_system: bool = Field(default=False, description="Whether this is a system permission (cannot be deleted)")
    is_active: bool = Field(default=True, description="Whether this permission is currently active")
    tags: List[str] = Field(default_factory=list, description="Tags for categorizing this permission")
