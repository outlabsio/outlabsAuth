...
class UserRoleMembershipCreate(BaseModel):
    ...
    assigned_by_id: Optional[str] = Field(None, description="User ID who assigned this role (for audit)")
    valid_from: Optional[datetime] = Field(None, description="Optional start date for role validity")
    valid_until: Optional[datetime] = Field(None, description="Optional end date for role validity")
...
class AssignRoleRequest(BaseModel):
    ...
    role_id: str = Field(..., description="Role ID to assign")
    valid_from: Optional[datetime] = Field(None, description="Optional start date for role validity")
    valid_until: Optional[datetime] = Field(None, description="Optional end date for role validity (e.g., 90-day contractor)")
