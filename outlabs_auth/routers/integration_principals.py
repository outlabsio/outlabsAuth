...
def get_integration_principals_router(
    ...
    """Generate admin routes for integration principals and owned system API keys."""
    if not getattr(auth.config, "enable_entity_hierarchy", False):
        return _get_platform_global_integration_principals_router(auth, prefix=prefix, tags=tags)
...
def _get_platform_global_integration_principals_router(
    ...
    """Generate the platform-global integration-principal surface for SimpleRBAC."""
    router = APIRouter(prefix=prefix, tags=tags or ["integration-principals"])
    ...
    async def delete_system_integration_principal(
        ...
        archived = await auth.integration_principal_service.archive_principal(
            ...
            principal_id,
            actor_user_id=_actor_user_id(auth_result),
            reason="Archived via platform-global integration principal route",
        )
        if not archived:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Integration principal not found")
        return None
