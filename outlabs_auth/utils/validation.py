...
from outlabs_auth.core.exceptions import InvalidInputError, ValidationError
...
def validate_email(email: str) -> str:
    ...
    if not email or not isinstance(email, str):
        raise InvalidInputError(message="Email is required", details={"field": "email"})
...
def validate_permission_name(permission: str) -> str:
    ...
    if not permission or not isinstance(permission, str):
        raise InvalidInputError(
            message="Permission name is required", details={"field": "permission"}
        )
...
def validate_positive_integer(
    value: int, field_name: str = "value", min_value: int = 1
) -> int:
    """
