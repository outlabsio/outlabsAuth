...
import type { RegisteredReadingsStatsResponse } from "~/stores/stats.store";
...
// Use real API data for registered readings
const { data, pending, error } = await useAsyncData<DataRecord[]>(
  async () => {
    try {
      const response = await statsStore.getRegisteredReadingsStats(props.period, props.range);
      ...
      console.error("Failed to fetch registered readings stats:", err);
...
<template>
  <UCard ref="cardRef" :ui="{ body: '!px-0 !pt-0 !pb-3' }">
    <template #header>
      <div>
        <p class="text-xs text-(--ui-text-muted) uppercase mb-1.5">Registered User Readings</p>
