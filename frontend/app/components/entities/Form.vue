...
const { data: entityTypes } = await useAsyncData(
  'entity-types',
...
const potentialParents = computed(() => {
  ...
  let filtered = allEntities.value.filter(
    e => e.entity_class === 'STRUCTURAL' && 
         e.status === 'active' && 
         (props.mode !== 'edit' || e.id !== props.entity?.id)
  )
...
const entityTypeOptions = computed(() => {
  ...
  // Get suggestions based on entity class
  const suggestions = selectedClass.value === 'STRUCTURAL' 
    ? ['platform', 'organization', 'division', 'department', 'team', 'unit']
    : ['admin_group', 'viewer_group', 'project_team', 'committee', 'task_force']
  
  // Combine existing types with suggestions
  const allTypes = new Set([...entityTypes.value, ...suggestions])
  ...
  <UForm :schema="schema" :state="state" @submit="onSubmit" class="space-y-8">
    ...
    <div class="space-y-6 w-full">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <template #description>
            <span class="text-xs text-muted-foreground">
              {{ selectedClass === 'STRUCTURAL' 
                ? 'Examples: platform, organization, department, team'
                : 'Examples: admin_group, viewer_group, project_team' }}
            </span>
          </template>
        </UFormField>
        ...
        >
          ...
          <template #description>
            <span class="text-xs text-muted-foreground">
              Leave empty to create a top-level entity
            </span>
          </template>
        </UFormField>
      </div>
    ...
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <UFormField 
          v-if="selectedClass === 'ACCESS_GROUP'"
          label="Maximum Members" 
          name="max_members"
          class="w-full"
        >
          <UInput 
            v-model="state.max_members" 
            type="number"
        ...
        <UFormField 
          ...
          name="status"
          required
          :class="{ 'md:col-span-2': selectedClass !== 'ACCESS_GROUP' }"
        >
          <URadioGroup
          ...
          >
            <template #label="{ option }">
              <div class="flex items-center gap-2">
                <UBadge 
                  :color="option.value === 'active' ? 'success' : 'neutral'" 
                  variant="subtle"
                  size="xs"
                >
                  {{ option.label }}
